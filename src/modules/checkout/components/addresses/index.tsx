  ...
  return (
    <div className="bg-white">
      <div className="text-xl-semi flex items-center gap-x-4 px-8 pb-6 pt-8">
        ...
        <h2>{checkout.addresses.shippingAddress}</h2>
      </div>
      {isEdit ? (
        <div className="px-8 pb-8">
          ...
          <div className="mt-6">
            <Checkbox
              label={checkout.addresses.sameBillingAddress}
              checked={checked}
              onChange={onChange}
            />
          </div>
          {!checked && (
            <div>
              <div className="text-xl-semi flex items-center gap-x-4 pb-6 pt-8">
                ...
                <h2>{checkout.addresses.billingAddress}</h2>
              </div>
              <BillingAddress />
            </div>
          )}
          ...
          >
            {checkout.addresses.continueDelivery}
          </Button>
        </div>
      ) : (
        <div>
                  ...
                  <div className="flex flex-col">
                    ...
                    {checked && (
                      <div className="flex items-center gap-x-2 mt-6">
                        ...
                        <span>{checkout.addresses.sameBillingAddress}</span>
                      </div>
                    )}
                  </div>
                  <div>
                    <button onClick={setEdit}>{checkout.addresses.edit}</button>
                  </div>
                </div>
              </div>
            ) : (
